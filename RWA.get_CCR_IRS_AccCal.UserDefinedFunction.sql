USE [Position_DB]
GO
/****** Object:  UserDefinedFunction [RWA].[get_CCR_IRS_AccCal]    Script Date: 2016/2/24 11:16:01 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE FUNCTION [RWA].[get_CCR_IRS_AccCal]
( 
...
(		
	...
	FROM(select distinct 账户# as 组合名称 from Position_DB.rwa.get_CCR_IRS(@balancedate)) as tmp1 )tmp2	
) as Acc1		
left join		
(		
	...
	FROM
	(	
		SELECT CONVERT(varchar(20), Balance_date, 111) as 结算日,AccId as 账户范围,账户# as 识别码	
			,sum(abs(notional))as 总规模, sum(notional)as 净规模
			,sum(presentvalue_CNY)as 合约价值合计,'' as 风险敞口,内评等级
			,PD,LGD,[UL_PD（金融机构）]
			,avg(CAST(datediff(day,balance_date,Settleday) AS FLOAT)/365) as 平均期限
		from Position_DB.rwa.get_CCR_IRS(@balancedate)	
		group by AccId,账户#,内评等级,pd,lgd,[UL_PD（金融机构）],Balance_date
	)as Acc3
	left join
	(
