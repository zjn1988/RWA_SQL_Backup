USE [Position_DB]
GO
/****** Object:  UserDefinedFunction [RWA].[get_mRWA_SVaR2]    Script Date: 2016/2/24 11:18:35 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE FUNCTION [RWA].[get_mRWA_SVaR2]
( 
...
/*账户层面*/
		...
		and RWA有效性=1 and 组合代码<>'0700' --大宗不分组合
	) as a
...
(	
select @BalanceDate as Balance_Date,SVaRType,RiskFactor,到期天数,CurveName
		,sum(ValuationFactor) as ValuationFactor,'0000'as 组合
	from 
	(
		select 
			(case when [标的代码]='000300.SH' then 'Equity'else (case
