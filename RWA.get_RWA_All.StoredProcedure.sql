USE [Position_DB]
GO
/****** Object:  StoredProcedure [RWA].[get_RWA_All]    Script Date: 2016/2/24 11:09:19 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [RWA].[get_RWA_All]
...
/*** RWA计算范围：1-SVaR;2-标准法;0-不计算 ***/
	...
	DECLARE @MaxID int;
	delete from Position_DB.[RWA].[mRWA_All] Where [Balance_Date]=@BalanceDate ;
	
------------------
	set @MaxID=(select isnull(max(ID),0) from Position_DB.[RWA].[mRWA_All]);

	insert into Position_DB.[RWA].[mRWA_All]
	(
		[ID]   ,
		BALANCE_DATE,
		组合代码,------------修改----------------
	...
	select 
		ROW_NUMBER() OVER (ORDER BY BALANCE_DATE ASC)+@MaxID AS ID
		,BALANCE_DATE
		,组合代码
		,AccId
		,SubAccId
		...
		,(case 
			...
			when ((secucategory='ETF' or secucategory='Fund') and (fundtype is null) or fundtype='货币市场型基金') then 2
					...
					/* 用标准法计算*/ 
					when left(AccId,2) = '大宗' then 2-- 大宗商品用标准法
					when left(AccId,2) = '公司' then 2-- 公司业务用标准法
					when AccId in('资金-打新','资金-其他') then 2--资金运营部DX定增、JJ基金、XG新股三个账户用标准法
					when AccId  = '证金-种子基金' then 2 --证金部种子基金
					when AccId = '股衍-新三板' then 2  --股衍新三板
					when AccId = '新三板-新三板' then 2  --2016年以后有新三板部其中新三板股票
					when AccId = '股衍-其他' then 2
					else 0 end) 
				end)
		end) as 'RWA有效性'
		...
		(
			...
			(
				...
				(
					...
					from
					(	
						select baseb.balance_date,baseb.subaccid,baseb.accid,baseb.组合代码,baseb.invarlimit,inCompany
						...
						from 
						(
							select base.balance_date,base.subaccid,base.accid,base.组合代码,base.invarlimit
							...
							from 
							(
								select base12.balance_date,base12.subaccid,base12.accid,base12.组合代码,base12.invarlimit
								...
								(
									select base1.balance_date,base2.subaccid
										,base2.Category as AccId,base2.组合代码 as 组合代码,base2.invarlimit
									...
									(
										select *
										from [Position_DB].[RWA].[get_AccInfo](@BalanceDate)  --账户映射表
									)as base2  
									...
									where 
										...
										and base2.subaccid <> 'YSPHGTCS' --股衍部沪港通测试户，量小且杂，市场组也不算
										...
										and base2.Subaccid <> 'YSPET2' --另一端在CSI的RQFII的ETF对冲																			
