...
(		
	...
	FROM
	(	
		select CONVERT(varchar(20), Balance_date, 111) as 结算日,'公司' as 账户范围,CounterpartyName as 识别码	
			,sum(abs(notional))as 总规模, sum(notional)as 净规模
			,sum(presentvalue_CNY)as 合约价值合计,'' as 风险敞口,内评等级,PD,LGD,[UL_PD（金融机构）]
			,avg(CAST(datediff(YEAR,balance_date,Settleday) AS FLOAT)) as 平均期限
		from Position_DB.rwa.get_CCR_IRS(@balancedate)	
		group by CounterpartyName,内评等级,pd,lgd,[UL_PD（金融机构）],Balance_date
	)as Acc3
	left join
	(
