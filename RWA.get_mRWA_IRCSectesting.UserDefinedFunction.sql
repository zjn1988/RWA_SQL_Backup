USE [Position_DB]
GO
/****** Object:  UserDefinedFunction [RWA].[get_mRWA_IRCSectesting]    Script Date: 2016/2/24 11:17:56 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE FUNCTION [RWA].[get_mRWA_IRCSectesting]
( 
...
select 
	...
	,IRCType,RiskFactor,RatingOrder,SysFactor,IdoFactor
	,case when MK_Duration is null then 0 else MK_Duration end MK_Duration
	,isnull(DurationOrder,'') DuartionOrder
...
(
	...
	(
		...
		,(case when 标的发行方 is NULL then 'Z-B5' --补充股票发行人!!!
			...
			else (case when  Innerrating is NULL then 'Z-B5' 
						else  Innerrating end) end
		)as'评级补全'
		,MK_Duration
		,IRCType
		...
		from ( 
				...
				from [Position_DB].[RWA].[mRWA_All] where Balance_date=@BalanceDate-- in('20150630','20150731','20150831','20150930','20151030','20151130','20151231') 
					...
					and accid like '%固收%'  ----------->修改！
			)c left join [CreditRisk].[Counterparty].[CounterpartyBasicInfo] d
			on CounterpartyName=标的发行方
		)a
	)b
